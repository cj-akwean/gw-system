...
| **Digital Wallet** *(Google Pay)* | **Implemented 2026-08-18.** One-tap official Google Pay button via PayMongo's `google_pay_card` method — on the review step it replaces the Swipe-to-pay (it is the single trigger; no second button). Token flow: PayMongo's JS-cardless sheet produces a tokenized `PAN_ONLY` payment token (VISA/Mastercard only; `CRYPTOGRAM_3DS` is PayMongo "coming soon") → create the `google_pay_card` Payment Method client-side with the **public key** + attach with `return_url`; issuer-auth redirects and in-sheet auth are both handled exactly like the Card flow (pending marker + `POST /api/payments/intent-status` recovery, method-agnostic — no new server work). Environment (TEST/PRODUCTION) derives solely from the public-key prefix; `gatewayMerchantId` in the tokenization spec is the PayMongo public key, and the Console merchant id (`NEXT_PUBLIC_GOOGLE_PAY_MERCHANT_ID`) appears only in `merchantInfo.merchantId` for live. Card-based: the ₱100k e-wallet cap does NOT block Google Pay. **Deployment gates**: PayMongo dashboard account capability (contact support if the Google Pay option is absent) + **Google Pay Console** business verification for live traffic — test mode works without it. Follow Google's brand guidelines for the button. Details: implementation-notes → Payments §8. |
