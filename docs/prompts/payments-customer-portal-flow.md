...
| **Digital Wallet** *(showcase feature — Google Pay)* | One-tap button via PayMongo's `google_pay_card` method. **Build note**: verify the exact token flow against PayMongo's Digital Wallets doc page before implementing — don't guess from memory. **Also**: Google Pay must be enabled as an **account capability** — if the option is absent from your PayMongo dashboard, contact PayMongo support. You must then pass **Google Pay Console** business verification + follow their brand guidelines before it works in live. Low build priority; mark "Coming soon" in the UI until verified rather than hiding it. |
