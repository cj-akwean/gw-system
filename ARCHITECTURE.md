...
- [x] Financial report page + Excel/PDF export *(2026-08-11)* — `FinancialReport` page (`/admin/financial-report`: summary stats + revenue-by-month table) + header actions; `App\Services\FinancialReportService` (single source of data for page/spreadsheet/PDF); `App\Exports\FinancialReportExport` (Laravel Excel, Summary + Revenue by Month sheets); PDF via dompdf (`pdfs/financial-report`); PDFs streamed as BinaryFileResponse from a temp file because Livewire can't serialize a plain dompdf response (details: docs/insights/implementation-notes.md → Admin Reports §4)
