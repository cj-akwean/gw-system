...
- [x] Digital Wallet (Google Pay) *(2026-08-18)* — real tap-to-pay flow via PayMongo's `google_pay_card` PM type + PaymentIntent direct API: official Google Pay button is the single trigger in the review step (replaces Swipe for Google Pay); tokenized client-side with the **public key** (PAN data never reaches Laravel); `PAN_ONLY` only, VISA/Mastercard, env derived from the key prefix (`pk_test_` → TEST); recovery reuses the existing card machinery (pending marker + `POST /api/payments/intent-status` + URL parsing — method-agnostic, zero new server work); card-based, so the ₱100k e-wallet cap does NOT block it; past-payments labeled "Google Pay". Live rollout gated on PayMongo account enablement + Google Pay Console business verification (details: docs/insights/implementation-notes.md → Payments §8)
