...
- **Webhook simulation**: `paymongo:simulate-payment {invoice?}` (CLI, dev tooling) fires the exact
  ...
  Both the CLI and the **dev-only `POST /api/dev/payments/simulate`** endpoint (Google Pay's in-page
  "Simulate payment (test)" button; **404 outside non-production**, ownership + payable guards, double-
  gated by the frontend `pk_test_` requirement) share the extracted `PaymentSimulationService`.
...
Quick reset of billing data for testing payment flows:

```bash
# Soft reset — wipes payments/invoices/readings/links, rebuilds from existing connections
php artisan test:seed-data

# Full reset — migrate:fresh + re-seed everything from scratch
php artisan test:seed-data --fresh
```

Creates 75 invoices (15 connections × 5 months: April–August) with 5 usage profiles
(steady_low/med/high, erratic, increasing). Status mix: 38 paid / 20 overdue / 17 unpaid.
All payments are offline (cash). Portal: `test@example.com` / `password`.
...
- [x] Google OAuth — "Sign in with Google" (client-side Google Identity Services) *(2026-08-21)* — frontend `GoogleSignInButton` (official GIS button, lazy script load, hidden without a Client ID) → ID token `POST /api/auth/google` → backend verifies via Google tokeninfo (audience + `email_verified`) → finds/creates user (`users.google_id` unique, nullable; `users.password` now nullable) → Sanctum token (same shape as `/api/login`); GitHub OAuth button removed; email-exists-without-google_id → 409 "log in with email" (anti-takeover policy); route `throttle:10,1,auth-google`; requires `GOOGLE_CLIENT_ID` (backend) + `NEXT_PUBLIC_GOOGLE_CLIENT_ID` (frontend) (details: docs/insights/implementation-notes.md → Auth §5; why: docs/insights/product-decisions.md §57)
...
- [x] Payment flow Screen 1 — Payment Method (three tappable cards: E-wallet, Card, Digital Wallet) *(2026-08-07)* — `/dashboard/pay?id={id}` (static-export-safe query route, also the GCash `return_url`); E-wallet card = Scan QR · QR Ph (client-side PM creation with the public key, `next_action.code.image_url` Base64 with `data:image/` guard, countdown driven by PayMongo's `expires_at` / backend `expiry_seconds`, sessionStorage resume, expired → new QR) + Open in GCash (attach with `return_url` → redirect; 4-hr window, no countdown); 15s poll + `GET /api/invoices/{id}` fallback → "Payment received" vs "not available" (403/404 only) vs "checking your payment" (transient probe failures keep polling — a failed probe never shows a definitive dead-end); E-wallet disabled > ₱100k; Card + Digital Wallet "Coming soon" (details: docs/insights/implementation-notes.md → Customer Portal §2)
...
- [x] Card form — collect details client-side, create Payment Method via PayMongo `/v1/payment_methods` with the **public key** (never through the Laravel backend; PCI note in the prompt file); 3DS redirect handling, re-fetch the intent server-side on return *(2026-08-07)* — Card card enabled; inline form in the review step (number/expiry/CVC + billing name/address/city/zip, phone+address2 optional, country PH, email prefilled); validates on Pay with inline errors (Luhn, MM/YY end-of-month, CVC 3–4); `startCard` = intent → PM (details + billing → also feeds webhook payer identity) → attach with `return_url` → 3DS `next_action.redirect.url` assign / `processing`+`succeeded` → pending note + poll / `awaiting_payment_method` → `last_payment_error` surfaced; return marker generalized `?from=redirect` (legacy `from=gcash` accepted); never mark paid on redirect — webhook only; bills > ₱100k become Card-only. **Redirect-return recovery + confirmed-payment feedback (2026-08-08):** PayMongo returns to a bare `/dashboard/pay` (query stripped; new tab observed — sessionStorage per-tab recovery failed); now `payment_intent_id`/`status` are parsed and resolved via new `POST /api/payments/intent-status` (intent → invoice, ownership-gated, `paid`/`confirmed`/`failed`/`processing`/`unknown`); pending marker rides sessionStorage **and** localStorage (1-h TTL); an unidentifiable return shows the checking panel — **never** "not available" (that panel is now 403/404-only on a known id); `paid`/`confirmed` → immediate "Payment received" panel (confirmed note polls until the webhook credits); `failed` → clear panel + Try again (details: docs/insights/implementation-notes.md → Customer Portal §4 addendum)
- [x] Tooltips: hover ⓘ on desktop, tap-to-toggle popover on touch — one consistent pattern, seed the `frontend-design` doc *(2026-08-08)* — shared `InfoTip` (`src/components/ui/info-tip.tsx`): single Radix Popover primitive gated on `pointerType === "mouse"` (hover opens after 200ms, leave closes after 120ms grace; tap/outside/Escape toggle natively; no matchMedia/SSR branching), `role="tooltip"` + `aria-describedby`, empty content renders nothing; wired into CVC (card form), Penalty rows (bill card + review step); 11 tests; pattern spec + rules: `docs/insights/frontend-design.md` (details: docs/insights/implementation-notes.md → Customer Portal §5)
- [x] Save-card checkbox + vaulting — deferred, added in the same release as the card form, never before *(2026-08-08)* — PayMongo Customer creation + `setup_future_usage` on PaymentIntent; `saved_payment_methods` local cache (brand/last4/expiry from PayMongo API, never raw card data); checkbox in CardForm (`saveCard: boolean` in CardPayload); webhook `payment.paid` extracts card source → fetches PM details → saves for all linked users; SavedCardSelector (radio list + CVC input); `POST /invoices/{id}/pay-with-saved` endpoint (validates ownership, updates CVC, creates intent, returns client_key for frontend attach); GET/DELETE on `/api/saved-payment-methods`; syncs from PayMongo on list; delete from PayMongo + local. Display: `{Brand} •••• {last4}  Exp {MM}/{YY}`. Data source: PayMongo only (webhook source + PM API for expiry). (details: docs/insights/implementation-notes.md → Customer Portal §6)
- [x] Portal self-registration + profile onboarding *(2026-08-08)* — signup is **email + password only** (`POST /api/register`, guest + `throttle:10,1,auth-register`; `users.name` now nullable, user created with `name = null`; auto-login response same shape as login, gains `avatar_id`); post-login redirect routes by profile completeness (`avatar_id` null → `/onboarding`); `/onboarding` wizard (resumable via `/user` + `/links`): Step 1 = avatar picker (`@kokonutui/avatar-picker`, adapted, 4 avatars) + username 3–20 chars → `PATCH /api/profile` (name = username, `avatar_id` 1–4) → Step 2 = link meter (account + meter number → `POST /api/links`; 404 "couldn't find an active connection…", 409 "already linked to another account"; **skippable** "I'll do this later") → Step 3 all-set → dashboard; step rail = `@blocks-so/onboarding-06` adapted (`OnboardingSteps`, lucide icons, parameterized); **one active link per connection** — `ConnectionLinkController::store` now guards via `pg_advisory_xact_lock` transaction (409 if actively linked to another user; own re-link idempotent; revoked links free); dashboard shows a **Link your meter** prompt (`LinkMeterPrompt`) until ≥1 active link. Backend 526/526, frontend 148/148, build static, lint baseline (details: docs/insights/implementation-notes.md → Customer Portal §7)
- [x] Auth-aware navigation + profile dropdown *(2026-08-09)* — landing hero nav swaps "Sign In" for a kokonutui-style `ProfileDropdown` when a session exists (name/email + circular avatar from `avatar_id`, rendered at natural size — the old `scale-[4]` in a 40px crop made it invisible); shared `DashboardHeader` brand links **back to the landing page** (`/`) with the landing's amber-dot wordmark; dropdown = Dashboard (hidden on `/dashboard` itself) · Settings (`/settings`) · **theme toggle item** (`useTheme()` hook) · Sign Out (→ landing `/`); **Settings page** (`/settings`, auth-guarded, same shell): edit profile (reused kokonutui `ProfileSetup` with `heading`/`subtitle` + `initialUsername`/`initialAvatarId` prefill → `PATCH /api/profile`) + meter management (link via extracted shared `LinkMeterForm`; **unlink via kokonutui `HoldButton`** — hold-gated so the confirm only fires after the full hold, `DELETE /api/links/{id}`); **Pay button → badtz-ui `SwipeButton`** ("Swipe to pay", `w-full`, busy states keep the disabled button); **theme toggle relocated** (floating one removed from layout — it overlapped nav/dropdown on tablet/phone): in-dropdown when signed in, in-flow on the hero nav for guests, floating on `/auth`; **mobile hamburger** (guests, below `lg`): nav items + **water-filled Sign In button** (compact centered pill — the sim mounts only while the menu is open) + theme in a dropdown panel; **performance pass (rounds 4–6)** — `OpeningAnimation` (loader) moved from layout to the landing page (portal routes render instantly), the **WebGL ripple removed by user decision** (round 6's native-support gate killed the splash on normal browsers; user chose deletion over restore — `canvasui/Ripple.tsx` gone, git-recoverable), **WaterCanvas always renders** with `frozen` prop (no DOM restructure when loadingComplete flips), **300ms pixel-scale intervals removed** from WaterCanvas + MetallicButton (ResizeObserver handles size), **Auth context value memoized** (`useMemo` — stable reference, no cascading re-renders), **ElasticLine rAF kept continuous** (a round-6 early-return "optimization" froze the spring bounce-back on release — reverted; the bounce is the per-frame spring rendering, verified in-browser grab→hold→release), **dark-mode circular reveal moved into `useTheme().toggle(origin?)`** (was only in `ThemeToggle`; the dropdown theme item now animates too — clipPath view transition 750ms, verified in-browser), hero image → `next/image` with `priority` (width/height/priority/preload plumbing; `images: { unoptimized: true }` in next.config — static export has no optimizer API and the webp is already optimized; a plain `<img>` throws 500 in dev under `output: "export"`), **dev switched to webpack** (`next dev --webpack` — Turbopack dev leaks ~5MB/s idle + holds its 662MB dev graph in RAM → 3–7GB; measured webpack: 208MB start, flat 760MB idle); a 14s LCP report traced to **7 orphaned node dev-server processes** (kill trees with `taskkill /T`, never just the wrapper) — fixed, machine back to one dev tree; `/auth` has a "← Back to home" link; avatars in `src/lib/avatars.tsx` (single source for picker + dropdown); **mouse-loop fix** — `useMousePosition`/`useDimensions`/`useElasticLineEvents` rAF-throttle + bail out on unchanged values (kills React 19 dev "Maximum update depth exceeded" under continuous mousemove); frontend 168/168, build static, lint baseline
- [x] Landing page: real nav + CTAs, Rates card, animated dot backdrop *(2026-08-09)* — nav = **Rates / How It Works / Contact** anchors (`HeroNavItem{label,href}`, desktop + mobile hamburger); hero CTAs wired: **Pay My Bill** (auth-aware `/dashboard` vs `/auth`) + **View Rates** (scrolls to `#rates`; `behavior:"smooth"` scrollIntoView was interrupted by hero re-renders → plain `scrollIntoView()` + CSS `scroll-smooth`); new **public `GET /api/rates`** (`RateService::publicPayload()` — current schedule by `effective_from` window + tiers sorted by min + current `PenaltyRule`; 404 when none in effect; `throttle:60,1,rates-index`); **Rates section** (`components/landing/rates-section.tsx`, id=`rates`): **`liquid-ocean`** three.js background (vengenceui registry, optimized — no grid/boats, `oceanFragments=12`, opacity .35, `dpr` 1, frameloop pauses off-screen; seeded PRNG replaced `Math.random` for React-19 purity lint; `RectAreaLightUniformsLib` import dropped — deprecated in three 0.185, no `.d.ts`) + pasted-Pricing-style card layout adapted (lucide `Check`, `border-t` instead of missing `Separator`, Button `lg` — no `xl` size); left card = live flat rate (₱10.00/m³) + features (2% penalty / 15-day grace / 60-day disconnection) with skeleton/loading + **office-contact fallback text** on fetch failure; right card = "How billing works" steps; **How It Works** (3 steps) + **Contact** sections (contact info = placeholder); **polka dots → `canvas-fractal-grid`** (cult-ui registry, landing only — wrote file directly after cult-ui 429'd the shadcn CLI): 2D canvas dot grid, theme-aware (`multiply`+dark dots light mode / `screen`+white dots dark mode via new `blendMode` prop), `enableGradient`/`enableNoise` off, 1.5px dots @ 20px, auto FPS degrade (setState moved into the rAF callback for the React-19 purity lint); **dashboard date line** — `CalendarDays` + `toLocaleDateString("en-PH")` under the sticky header (auth-guarded pages now render a `PageLoader` instead of blank `null` — no more white flash during redirects); new deps: `three@0.185.1`, `@react-three/fiber@9.7.0`, `@types/three` (three dropped bundled types in r156); backend 532/532, frontend 172/172, tsc + lint clean (details: docs/insights/implementation-notes.md → Landing Page §1)
- [ ] Digital Wallet (Google Pay) — **reverted to "Coming soon" *(2026-08-19)*** — the frontend flow was implemented (2026-08-18) but PayMongo sandbox offers no `test_url` simulator for `google_pay_card` (unlike QR Ph), so the dev-only simulate harness bypasses PayMongo entirely (local-only `ProcessPayMongoWebhook` dispatch). This means Google Pay payments cannot be verified end-to-end in the sandbox — nothing appears in the PayMongo webhook dashboard. The `google_pay_card` PM type and backend wiring remain in place; the Digital Wallet card in the portal UI now renders disabled with "Coming soon". Re-enable when a testable flow is available (ngrok + real Google Pay test card, or PayMongo adds a simulator). (details: docs/insights/implementation-notes.md → Payments §8)

### Admin Panel (Filament)
- [x] Dashboard with key metrics (customers, unpaid invoices, revenue) — `DashboardMetricsService` + `MetricsOverview`/`RevenueChart`; customers = active connections, revenue = all `Payment.amount` by `paid_at`, outstanding = unpaid + overdue; needs-attention + recent-payments widgets, collection rate + AR 90+ aging + trend deltas (2026-08-22) (details: docs/insights/implementation-notes.md → Admin Panel §1)
...
- [x] Financial report → Accounting & Finance module *(2026-08-17)* — `FinancialReport` page (`/admin/financial-report`, nav "Accounting & Finance", `navigationSort 100` → sidebar below resources) rebuilt as formal accounting content: date-range picker (monthly/quarterly/yearly/custom, `#[Url]` state), receivables-vs-collections summary, AR aging by due-date buckets (0–30/31–60/61–90/90+, count + outstanding + stored penalties), cash-vs-accrual statement of income (NOI = gross billed + misc − collections), and an independently filterable payment reconciliation EmbeddedTable (method + date). Dashboard keeps the operational KPIs/graph; `App\Services\FinancialReportService::build(from,to)` remains the single data source; Excel now 4 sheets (Summary, AR Aging, Income Statement, Payments Ledger); PDF via dompdf (landscape, same BinaryFileResponse temp-file pattern). Misc income is penalty amounts only — reconnection/setup fees not tracked yet (details: docs/insights/implementation-notes.md → Admin Reports §4)

### Branding *(2026-08-11)*
- [x] App named "Guinobatan Waterworks" end-to-end — `APP_NAME="Guinobatan Waterworks"` (.env + .env.example; also fixes mail from-name, session/cache prefixes); explicit `->brandName()` + `->favicon('/favicon.svg')` on the Filament panel; branded `welcome.blade.php` at `/` (was the default Laravel page); frontend metadata title + branded `src/app/icon.svg` (Next.js default favicon removed; unused `next.svg`/`vercel.svg`/etc. deleted) (details: docs/insights/implementation-notes.md → Branding §1)
- [x] Filament light-mode soft-slate background *(2026-08-11)* — `.fi-body` #f1f5f9, topbar/mobile sidebar #f8fafc, injected via `PanelsRenderHook::HEAD_START` renderHook (no Vite/theme build needed); dark-mode values re-declared after so dark theme is untouched (details: docs/insights/implementation-notes.md → Branding §2)
...
- [x] Inventory tab — `InventoryItemResource` + `InventoryCategoryResource` (new **Inventory** sidebar group); categories seeded like barangays but admin-manageable (incl. inline category creation on the item form); items are free-text names (brand/spec/price live in the name — no structured price field); stock moves only through `App\Services\InventoryService` (DB txn + `lockForUpdate`, overdraw blocked, ledger always written, `recorded_by` audit, no edit/delete on transactions); item view has Add Stock / Remove Stock header actions + append-only Transactions tab; delete blocked for items with history and categories in use; status badge (Low stock / OK) + category & low-stock-only filters (details: docs/insights/implementation-notes.md → Inventory §1)
- [x] Low-stock notifications — immediate boundary-crossing alert (delivered **synchronously** via `Notification::sendNow` — same bell/hub payload as AdminNotifier but no queue-worker dependency, deliberate deviation documented in product-decisions §49; no spam while continuously low; re-alerts after restock) + daily `inventory:check-low-stock` digest (07:00 PH, one aggregate notification, `--dry-run` / `--fix`); seeder emits the digest so seeded-low items ring the bell immediately; schedule pinned by `ScheduleTest` (details: docs/insights/implementation-notes.md → Inventory §1–2)
