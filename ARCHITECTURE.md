...
- [x] Auth-aware navigation + profile dropdown *(2026-08-09)* — landing hero nav swaps "Sign In" for a kokonutui-style `ProfileDropdown` when a session exists (name/email + square avatar from `avatar_id`); shared `DashboardHeader` brand links **back to the landing page** (`/`) with the landing's amber-dot wordmark; dropdown = Profile (`/onboarding`) · Dashboard (hidden on `/dashboard` itself) · Settings (`/settings`) · Sign Out (→ landing `/`); **Settings page** (`/settings`, auth-guarded, same shell): edit profile (reused kokonutui `ProfileSetup` with `initialUsername`/`initialAvatarId` prefill → `PATCH /api/profile`) + meter management (link via extracted shared `LinkMeterForm`, unlink via new `DELETE /api/links/{id}` frontend `unlinkApi` with inline two-step confirm, backend `ConnectionLinkController::destroy` already existed); `/auth` has a "← Back to home" link; avatars in `src/lib/avatars.tsx` (single source for picker + dropdown); **mouse-loop fix** — `useMousePosition`/`useDimensions`/`useElasticLineEvents` rAF-throttle + bail out on unchanged values (kills React 19 dev "Maximum update depth exceeded" under continuous mousemove); frontend 164/164, build static, lint baseline
