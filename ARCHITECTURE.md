...
- [ ] **Payer identity captured + shown on receipt** *(planned 2026-08-06)* — `payments.payer_name/email/phone` (nullable, from webhook `attributes.billing`; offline rows stay NULL); email body gains Customer (registered_name), Account No., Meter No., Payer rows (the PDF attachment already shows customer identity). Rationale: product-decisions §26.
