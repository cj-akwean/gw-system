...
// Deterministic PRNG so the wave field is stable across renders (purity-safe
// replacement for Math.random in three.js geometry setup).
...
function OceanMesh({
    ...
    const { geometry, waves } = useMemo(() => {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            waveData.push({
                ...
                ang: Math.PI * 2,
                amp: rand() * waveAmplitude,
                speed: 0.03 + rand() * waveSpeed,
            });
        }
    ...
    // eslint-disable-next-line react-hooks/immutability -- three.js animation mutates the geometry-driven wave table by design
    useFrame(() => {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            ...
            // eslint-disable-next-line react-hooks/immutability -- three.js animation advances the wave phase by design
            wave.ang += wave.speed;
        }
    ...
    const boats = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            items.push({
                position: [x, 0, z],
                scale: [sX, sY, sX],
                rotationY: (rand() * 360 * Math.PI) / 180,
                vel: 1 + rand() * 4,
                amp: 1 + rand() * 6,
                pos: rand() * 0.2,
            });
