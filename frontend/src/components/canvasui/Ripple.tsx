...
export interface RippleProps extends RippleOptions {
  ...
  onRippleReady?: (ripple: { splash: (x: number, y: number, strength?: number) => void }) => void;
}

const emptySubscribe = () => () => {};

export function Ripple({ children, className, style, onRippleReady, ...options }: RippleProps) {
  ...
  useEffect(() => {
    ...
    const inst = createRipple(
      { source, content, output },
      initialOptions,
    );
    instanceRef.current = inst;
    if (native && !inst) setFailed(true);
    if (inst && onRippleReady) {
      onRippleReady({ splash: inst.splash });
    }
    return () => {
      instanceRef.current?.destroy();
      instanceRef.current = null;
    };
  }, [initialOptions, native, onRippleReady]);
